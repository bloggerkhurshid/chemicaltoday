...
import Cs from './CaseStudy';
class CaseStudyCommonUtils {
    ...
    static async fetchPostCommonUtils() {
        try {
            console.log("yes")
            const response = await fetch(`${BASE_URL}/casestudy/post_common_utils`);
            // console.log("sdfdsfs")
            ...
            var fp_posts = data.featured_post.map(post => new Cs(post.id ,post.title, post.published_by, post.published_date, post.web_image_url, post.mob_image_url, post.thumbnail_image_url, post.heading_1, post.content, post.visibility, post.related_posts, post.created_at, post.created_by)) 
...
console.log("hb")
console.log(h_b)
        var post_common_utils = new CaseStudyCommonUtils( data.id,h_b,fp_posts,f_ad1,f_ad2,f_ad3,f_ad4,data.latest_updates1,data.latest_updates2,data.latest_updates3,data.latest_updates4,i_ad1,i_ad2,i_ad3,i_ad4)
...
export default CaseStudyCommonUtils;
